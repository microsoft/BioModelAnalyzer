...
*)
    ...
    let (extra_asserts, zr) = tr expr
    (extra_asserts, z.MkAdd(zr, z.MkDiv(z.MkRealNumeral(1), z.MkRealNumeral(2))))
...
let find_bifurcation (network : QN.node list) range =
    ...
    if sat = LBool.True then
        ...
        if Log.level(1) then Log.log_debug "No initial fixpoint when looking for bifurcation"
