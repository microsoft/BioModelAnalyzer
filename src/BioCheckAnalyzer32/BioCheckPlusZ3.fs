...
let constraint_for_target_function_boolean (qn:QN.node list) (node : QN.node) inputnodes listOfValues previousRange current prev (z : Context) = 
    ...
    let vals_in_list = List.sort (List.distinct list_of_actual_next_vals)
    if not ((List.length vals_in_list) = (List.length listOfValues)) || List.exists (fun (i, j) -> i=j) (List.zip vals_in_list listOfValues) then
        let comb_list = List.zip list_of_new_transition_option_vars list_of_actual_next_vals
        let short_comb_list = List.filter (fun (c, v) -> List.contains v listOfValues) comb_list
