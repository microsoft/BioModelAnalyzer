var _this = this;
describe("BMA.Model.AppModel", function () {
    var appModel;
    ...
    it("should reset model to correct serializedModel", function () {
        ...
        _this.model = new BMA.Model.BioModel(ml.model.name, variables, relationships);
        _this.layout = new BMA.Model.Layout(containers, variableLayouts);

        var serializedModel = JSON.stringify(ml);
        appModel.Reset(serializedModel);
        expect(appModel.BioModel).toEqual(_this.model);
        expect(appModel.Layout).toEqual(_this.layout);
        expect(appModel.ProofResult).toEqual(undefined);
    });

    it("shouldn't reset model when serializedModel isn't correct", function () {
        var ml = {
            "model": {
                "name": "model 1",
                "variables": [
    ...
    it("should turn model as new when serializedModel is undefined", function () {
        appModel.BioModel = biomodel;
        appModel.Layout = layout;
        appModel.ProofResult = proof;
    ...
    it("should turn model as new when serializedModel is null", function () {
        appModel.BioModel = biomodel;
        appModel.Layout = layout;
        appModel.ProofResult = proof;
