...
(function (BMA) {
    function ParseXmlModel(xml, grid) {
        ...
        var modelVars = ($variables.map(function (idx, elt) {
            ...
            return new Model.Variable(parseInt($elt.attr("Id")), parseInt(containerId), $elt.children("Type").text(), $elt.attr("Name"), parseInt($elt.children("RangeFrom").text()), parseInt($elt.children("RangeTo").text()), $elt.children("Formula").text());
        }).get());
        ...
        var modelRels = ($relations.map(function (idx, elt) {
            var $elt = $(elt);
            return new Model.Relationship(parseInt($elt.attr("Id")), parseInt($elt.children("FromVariableId").text()), parseInt($elt.children("ToVariableId").text()), $elt.children("Type").text());
        }).get());
        ...
        var containers = ($containers.map(function (idx, elt) {
            ...
            return new Model.ContainerLayout(parseInt($elt.attr("Id")), $elt.attr("Name"), parseInt(size), parseInt($elt.children("PositionX").text()), parseInt($elt.children("PositionY").text()));
        }).get());
        ...
        var varLayouts = $variables.map(function (idx, elt) {
            ...
            if (modelVars[idx].Type === "MembraneReceptor") {
                ...
                var p = SVGHelper.GeEllipsePoint((cnt.PositionX + 0.5) * grid.xStep + grid.xOrigin + (cnt.Size - 1) * grid.xStep / 2 + 2.5 * cnt.Size, (cnt.PositionY + 0.5) * grid.yStep + grid.yOrigin + (cnt.Size - 1) * grid.yStep / 2, 107 * cnt.Size, 127 * cnt.Size, x, y);
            ...
            return new Model.VariableLayout(id, x, y, Number.NaN, Number.NaN, parseFloat(angle));
        }).get();

        return {
            Model: new Model.BioModel($xml.children("Model").attr("Name"), modelVars, modelRels),
            Layout: new Model.Layout(containers, varLayouts)
        };
    }
