...
module BMA {
    export module Elements {
        export class Element {
            private type: string;
            private renderToSvg: (x: number, y: number) => SVGElement;
            ...
            public get RenderToSvg(): (x: number, y: number) => SVGElement {
            ...
            constructor(type: string, renderToSvg: (x: number, y: number) => SVGElement, description: string, iconUrl: string) {
                this.type = type;
        ...
        export class ElementsRegistry {
            ...
            private CreateSvgElement(type: string, x: number, y: number, scale: number = 1.0) {
            ...
            private CreateSvgPath(data: string, color: string, x: number = 0, y: number = 0, scale: number = 1.0) {
                var elem = <SVGPathElement>this.CreateSvgElement("path", x, y, scale);
            ...
            constructor() {
                ...
                this.elements.push(new Element(
                    "MembraneReceptor",
                    function (x, y) {
                        return that.CreateSvgPath("M9.9-10.5c-1.4-1.9-2.3,0.1-5.1,0.8C2.6-9.2,2.4-13.2,0-13.2c-2.4,0-2.4,3.5-4.8,3.5c-2.4,0-3.8-2.7-5.2-0.8l8.2,11.8v12.1c0,1,0.8,1.7,1.7,1.7c1,0,1.7-0.8,1.7-1.7V1.3L9.9-10.5z",
                            "#3BB34A", x, y);
                    },
                    "Membrane Receptor",
                    "images/receptor.png"));
