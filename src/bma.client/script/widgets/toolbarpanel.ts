...
(function ($) {
    $.widget("BMA.toolbarpanel", {
            ...
            this.buttons.each(function (ind) {
                try {
                    ...
                    if (command !== undefined) {
                        $(this).bind("click", function (e) { window.Commands.Execute(command, commandParameter); });
                    }
                } catch (ex) {
        ...
        addDivButton: function (div: JQuery) {
...
interface ToolBarPanel extends JQuery {
    addDivButton();
}

interface JQuery {
    toolbarpanel(): ToolBarPanel;
    toolbarpanel(settings: Object): ToolBarPanel;
}