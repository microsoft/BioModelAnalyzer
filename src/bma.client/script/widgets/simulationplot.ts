...
(function ($) {
    $.widget("BMA.simulationplot", {
        ...
        _create: function () {
            ...
            this.chartdiv = $('<div id="chart" data-idd-plot="plot" style="width: 100%; height: 160px;"></div>').appendTo(that.element);
            ...
            for (var i = 0; i < that.options.data.length; i++)
                $('<div></div>').attr("id", "polyline" + i).attr("data-idd-plot", "polyline").width("100%").height(160).appendTo(this.chartdiv);
        ...
        refresh: function () {
            ...
            if (options.data !== undefined) {
                for (var i = 0; i < options.data.length; i++) {
                    var y = options.data[i];
                    var polyline = that._chart.get(that.chartdiv.children().eq(i).attr("id"));
                    polyline.stroke = that.options.colors[i].Color;
                    polyline.isVisible = that.options.colors[i].Seen;
