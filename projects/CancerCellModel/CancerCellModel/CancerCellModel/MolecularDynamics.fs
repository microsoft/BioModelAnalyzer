...
type MolecularDynamics() = 
    ...
    static member repulsive_force(cell1: Cell, cell2: Cell) =
        ...
        let fmod = if (dr < 1.) then
                        (!ModelParameters.RepulsiveForce).Y(dr)
                    else
                        0.
...
(*
        // the code below is another way to compute repulsive-attractive forces -
        // as a gradient of interaction energy
        // but it did not work because it's too complicated
