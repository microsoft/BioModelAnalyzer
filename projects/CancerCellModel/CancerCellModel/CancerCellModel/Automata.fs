...
type GlobalState() =
    ...
    let mutable G0_cells_num = 0    // the number of cells in quiescent state (G0)
    let mutable G1_cells_num = 0    // the number of cells in G1 phase
    let mutable S_cells_num = 0     // the number of cells in S phase
    let mutable G2M_cells_num = 0   // the number of cells in G2/M phase
    ...
    member this.NumofG0Cells with get() = G0_cells_num and set(x) = G0_cells_num <- x
    member this.NumofG1Cells with get() = G1_cells_num and set(x) = G1_cells_num <- x
    member this.NumofSCells with get() = S_cells_num and set(x) = S_cells_num <- x
    member this.NumofG2MCells with get() = G2M_cells_num and set(x) = G2M_cells_num <- x
...
type Automata() =
    ...
    static member divide(glb: GlobalState)(cell: Cell) = 
        ...
        let non_stem_daughter = new Cell(type2, cell.Generation + 1, loc1, r1, cell.Density)    // check type1 and type2 in non_stem_daughter and stem_daughter, 
        let stem_daughter = new Cell(type1, cell.Generation + 1, loc2, r2, cell.Density)        // might have to change non_stem_daughter -> type2 and stem_daughter -> type1
    ...
    static member private can_divide(cell: Cell, glb: GlobalState) =
        ...
        if (cell.WaitBeforeS > 0) then
            ...
            // the probability of cell division is calculated
            // as the product of three functions (they all range between 0 and 1)
                // one takes as its argument the concentration of oxygen
                // the second takes the concentration of glucose
                // the third takes the cell packing density
    ...
    static member private should_goto_necrosis(cell: Cell, glb: GlobalState) = 
        ...
        // we calculate the probability of death based on teo parameters independently
        ...
        // then the cell decides to stay alive
    ...
    static member initialise_new_cell(cell: Cell) =
        match cell.Type with
        | Stem -> cell.WaitBeforeS <- uniform_int(ModelParameters.NonStemWaitBeforeCommitToDivideInterval)
        | NonStem -> cell.WaitBeforeS <- uniform_int(ModelParameters.NonStemWaitBeforeCommitToDivideInterval)
        | _ -> raise(InnerError(sprintf "Error: new cell in state %s" (cell.Type.ToString())))
    ...
    static member compute_action(glb: GlobalState) (cell: Cell) = 
        try
            ...
            else if (cell.CellCycleStage = G1) then
                if cell.WaitBeforeS > 0 then
...
//                        cell.Type <- Stem
    ...
    static member do_step(cell: Cell) =
        if (cell.Action = GotoCytokinesis && (cell.DivideAction = StemSymmetricDivision || cell.DivideAction = StemAsymmetricDivision)) then
            cell.WaitBeforeS <- uniform_int(ModelParameters.StemWaitBeforeCommitToDivideInterval)
            cell.StepsAfterLastDivision <- 0
        else if (cell.Action = GotoCytokinesis && cell.DivideAction = NonStemDivision) then
...
//        if (cell.DivideAction = StemSymmetricDivision || cell.DivideAction = StemAsymmetricDivision) then
//            cell.WaitBeforeDivide <- uniform_int(ModelParameters.StemWaitBeforeDivisionInterval)
//            cell.StepsAfterLastDivision <- 0
//        else if cell.DivideAction = NonStemDivision then
