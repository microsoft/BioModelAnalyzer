...
let rec eval_expr_int (node:var) (range:Map<var,int*int>) (e : expr) (env : Map<var, int>) =
    ...
    let rec eval_expr_int e env =
        ...
        | Var v ->
            ...
            let scale = if (v_min<>v_max) then (node_max - node_min) / (v_max - v_min) else 0
            let displacement = node_min - v_min
            match Map.tryFind v env with
            | Some x -> float( (x + displacement) * scale ) //float(x)
            | None -> float(node_min) // 0.0 
        | Const c -> float(c)
    ...
    let res = int (eval_expr_int e env)
