...
// Set of all unit tests across T2
            ...
            if !catch_exceptions then
                try let b = f()
                    if b<>stored then
                          ...
                          Printf.printf "Stored = %b, Current = %b\n" stored b
                          failed := (info,b,stored) :: !failed;
                    b
                with e ->
                    Printf.printf "Regression: %s,%d\n" file line
                    Printf.printf "RAISED EXCEPTION!!!!!\n"
                    Printf.printf "Stored = %b, Current = %s\n" stored
                                   "exception"
...
let inline register_test s f =
    ...
    tests := !tests @ [(s,f,(fn,n,k))]


let inline register_expensive_test s f =
    if (!expensive_tests) then
        ...
        let k = cf.GetFileLineNumber()
        let fn = cf.GetFileName()
        tests := (s,f,(fn + "(expensive)",n,k)) :: !tests
