...
module nuSkinCell
// the SkinFxd / Skin models 
// for multiple activators and inhibitors, use sum instead of ave for the target functions
...
let bcat_target bcatexp axin = Expr.Max (Expr.Const 0, Expr.Minus(Expr.Var bcatexp, Expr.Var axin))
//let bcat_target axin = Expr.Max(Expr.Const 0, Expr.Minus(Expr.Const 0, Expr.Var axin))
...
//let axin_target dsh = Expr.Max(Expr.Const 0, Expr.Minus(Expr.Const 0, Expr.Var dsh))

// dsh_target frizzled = frizzled
let dsh_target frizzled = Expr.Var frizzled
//let dsh_target frizzled wntext = Expr.Min(Expr.Var frizzled, Expr.Var wntext)

// frizzled_target wntext = wntext
let frizzled_target wntext = Expr.Var wntext
//let frizzled_target N = Expr.Const N
...
type InternalSkinCell(N, notch, x, y, z) =
    ...
    let bcat : QN.node = { var = bcat_id; f = bcat_target bcatexp_id axin_id; inputs = [bcatexp_id; axin_id];
                               name = "bcat" + subscript }
    (*
    let bcat : QN.node = { var = bcat_id; f = bcat_target axin_id; inputs = [axin_id];
                               name = "bcat" + subscript }
    *)
    ...
    let axin : QN.node = { var = axin_id; f = axin_target cklalpha_id dsh_id; inputs = [cklalpha_id; dsh_id];
                               name = "axin" + subscript }
    (*
    let axin : QN.node = { var = axin_id; f = axin_target dsh_id; inputs = [dsh_id];
                               name = "axin" + subscript }
    *)
    ...
    interface SkinCell with
        ...
        member this.Nodes =
            ...
            [delta ; wnt ; notch_ic ; wntext ; deltaext; bcatexp; cklalpha; frizzled; dsh; axin; bcat; gt1; gt2; p21]
            //[delta ; wnt ; notch_ic ; wntext ; deltaext; frizzled; dsh; axin; bcat; gt1; gt2; p21]
