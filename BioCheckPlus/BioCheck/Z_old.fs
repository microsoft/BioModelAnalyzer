...
let rec expr_to_z3 (node:QN.node) range expr (var_names : Map<int, string>) time (z : Context) =
    ...
    | Var v ->
        ...
        //let scale = z.MkRealNumeral( ((node_max - node_min) / (v_max - v_min)):int ) 
        
        //Begin <-- Edit by Qinsi Wang
        // Need to make sure that v_max - v_min is not equal to 0
        let scale = if (v_max - v_min) <> 0 then z.MkRealNumeral( ((node_max - node_min) / (v_max - v_min)):int )
                    else z.MkRealNumeral 0
        //End <-- small logical flaw
    ...
    | Ceil e1 -> 
        ...
        let z1 = expr_to_z3 node range e1 var_names time z
        // Begin <-- Edit by Qinsi Wang
        // the format of the parameter of z.MkRealNumeral is changed
        // 1/2 is changed to 99/100 to insure the correctness of ceiling function
...
let assert_target_function_matt (node: QN.node) var_names bounds start_time end_time (z : Context) =
    ...
    let output_var = sprintf "%s^%d" node.name end_time
    let lhs = z.MkConst(z.MkSymbol output_var, z.MkIntSort())
    let cnstr = z.MkEq(lhs, rhs)
    Log.log_debug (z.ToString cnstr)
    z.AssertCnstr cnstr

let assert_target_function (node: QN.node) var_names bounds start_time end_time (z : Context) =
    ...
    let T_applied = z.MkToInt(expr_to_z3 node bounds node.f var_names start_time z)
...
let assert_bound_pair (node : QN.node) (lower : int , upper : int) (time : int) (z : Context) =
...
let unroll_qn_bounds qn bounds var_names start_time end_time z =
    ...
    for node in qn do
        assert_target_function node var_names bounds start_time end_time z

    // Now assert the bounds...
    for node in qn do
        assert_bound_pair node (Map.find node.var bounds) start_time z
        assert_bound_pair node (Map.find node.var bounds) end_time z
...
let assert_states_equal (qn : QN.node list) start_time end_time (ctx : Context) =
    ...
    for node in qn do
        let start_name = sprintf "%s^%d" node.name start_time
        let end_name = sprintf "%s^%d" node.name end_time
