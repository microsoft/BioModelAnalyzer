...
type ModelParameters() =
    ...
    static let mutable stem_division_prob_cell_density_param = ref (LogisticFunc(min = Point(0.65*density_limits.Max, 0.), max = Point(0.45*density_limits.Max, 1.)))
    ...
    static let mutable nonstem_division_prob_cell_density_param = ref (LogisticFunc(min = Point(0.45*density_limits.Max, 0.), max = Point(0.3*density_limits.Max, 1.)))
    static let mutable nonstem_tostem_prob_param = ref (ShiftExponentFunc(p1 = Point(0., 1.), p2 = Point(1000., 0.01), ymin = 0.))
    static let mutable nonstem_death_prob_o2_param = ref (LogisticFunc(min = Point(20., 0.), max = Point(10., 1.)))
    static let mutable nonstem_death_prob_density_param = ref (LogisticFunc(min = Point(0.75*density_limits.Max, 0.), max = Point(density_limits.Max, 1.)))
    ...
    static member StemDivisionProbO2 with get() = stem_division_prob_o2_param
                                          and set(p) = stem_division_prob_o2_param <- p

    static member NonStemDivisionProbO2 with get() = nonstem_division_prob_o2_param
                                              and set(p) = nonstem_division_prob_o2_param <- p

    // the probability of the transition from a stem to non-stem with memory state
    static member StemToNonStemProb with get() = stemto_nonstem_prob_param
    ...
    static member NonStemToStemProb with get() = nonstem_tostem_prob_param
    ...
    static member NonStemDeathProbO2 with get() = nonstem_death_prob_o2_param
                                      and set(p) = nonstem_death_prob_o2_param <- p
    ...
    static member StemDivisionProbDensity with get() = stem_division_prob_cell_density_param
                                                    and set(x) = stem_division_prob_cell_density_param <- x

    static member NonStemDivisionProbDensity with get() = nonstem_division_prob_cell_density_param
                                                    and set(x) = nonstem_division_prob_cell_density_param <- x
